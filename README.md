...
- `tests/Teyemer.Tests`: 매일 공통 활성 시간 경계, 상태 전이, 레거시 설정 변환, 설정 복구, 명령 인용 테스트
...
트레이 메뉴는 상태/남은 시간, 즉시 운동, 30분·오늘 일시정지, 설정, 종료를 제공합니다. 눈 운동 알림은 항상 사용되며 알림 주기는 분 단위 입력으로 설정하고 운동은 20초 동안 진행됩니다. 설정 창은 일반, 활성 시간, 사용자 알람 탭으로 구성됩니다. 사용자 알람은 목록에서 관리하고 별도의 모달 설정 창에서 1~1440분 간격의 주기형 또는 매일 특정 시각형과 표시 문구를 설정할 수 있습니다. 시·분 Picker로 지정하는 매일 공통 활성 시간과 자정 통과 구간도 지원합니다. 활성 시간 밖에서는 눈 운동 주기를 중단하고 다음 활성 시간이 시작되면 새로운 전체 주기를 시작합니다. 사용자 알람은 잠금·장시간 부재 중 누적하지 않습니다. 기존 요일별 활성·비활성 설정 파일은 로드 시 매일 공통 활성 시간 형식으로 변환됩니다. 일반 설정에서 청회색 기반 다크 모드와 알림 자동 닫힘 시간(1~60초, 기본 30초)을 설정할 수 있습니다. Windows 11에서는 메인 창에 Mica, 운동 창에 transient system backdrop을 적용하고 모든 콘텐츠를 반투명 유리 카드로 표시합니다. 지원되지 않는 환경에서는 동일 팔레트의 불투명 배경으로 안전하게 대체됩니다. 알림은 자동 또는 사용자 동작으로 닫힐 때 페이드아웃됩니다. 잠금·절전과 10분 사용자 부재 중 알림을 억제하고 복귀 시 새 주기를 시작합니다.
